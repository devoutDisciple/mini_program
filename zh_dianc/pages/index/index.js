...
Page({
  ...
  onLoad: function(a) {
    var e = this,
      t = wx.getStorageSync("bqxx");
    console.log(t), this.setData({
      bqxx: t
    }), wx.getSystemInfo({
      success: function(a) {
        console.log(a.windowWidth), console.log(a.windowHeight), e.setData({
          height: a.windowHeight / a.windowWidth * 750 - 520
        });
      }
    }), wx.login({
      success: function(a) {
  ...
  reload: function(a) {
    var n = this,
      r = util.formatTime(new Date()).slice(11, 16);
    ...
    console.log(t), app.util.request({
      ...
      success: function(a) {
        for (var t = 0; t < a.data.length; t++)
          for (var e = 0; e < a.data[t].goods.length; e++) a.data[t].goods[e].xs_num = Number(a.data[t].goods[e].xs_num),
  ...
  addShopCart: function(a) {
    ...
    var e = a.currentTarget.dataset.itemIndex,
      s = a.currentTarget.dataset.parentindex,
      o = a.currentTarget.dataset.gwcindex,
      d = a.currentTarget.dataset.ggindex;
    if (null != d) {
      this.data.dishes[s].goods[e].one++, this.data.dishes[s].goods[e].gg[d].num++;
      var i = "a" + e + "b" + s + "c" + this.data.dishes[s].goods[e].gg[d].id;
      if (2 == this.data.types) var n = this.data.dishes[s].goods[e].gg[d].cost;
      else n = this.data.dishes[s].goods[e].gg[d].cost;
      var r = this.data.dishes[s].goods[e].box_fee;
      console.log("餐盒费是：" + r);
      var l = Number(t.data.dishes[s].goods[e].num),
        c = this.data.dishes[s].goods[e].gg[d].num,
        g = this.data.dishes[s].goods[e].name + this.data.dishes[s].goods[e].gg[d].name,
        h = this.data.dishes[s].goods[e].id,
        u = this.data.dishes[s].goods[e].img,
        p = this.data.store,
        m = {
          ggindex: d,
          money: n,
      ...
      i = "a" + e + "b" + s;
      if (2 == t.data.types) n = t.data.dishes[s].goods[e].wm_money;
      else n = t.data.dishes[s].goods[e].money;
  ...
  decreaseCart: function(a) {
    ...
    var u = a.currentTarget.dataset.itemIndex,
      p = a.currentTarget.dataset.parentindex;
    console.log(u, p), console.log(p, h.data.dishes[p].goods[u].id), app.util.request({
      ...
      success: function(a) {
        if (console.log(a), 0 != a.data.length) wx.showModal({
          ...
          var t = "a" + u + "b" + p;
          if (2 == h.data.types) var e = h.data.dishes[p].goods[u].wm_money;
          else e = h.data.dishes[p].goods[u].money;
          var s = h.data.dishes[p].goods[u].box_fee;
          console.log("餐盒费是：" + s);
          var o = Number(h.data.dishes[p].goods[u].num),
            d = h.data.dishes[p].goods[u].one,
            i = h.data.dishes[p].goods[u].name,
            n = h.data.dishes[p].goods[u].id,
            r = h.data.dishes[p].goods[u].img,
            l = h.data.store,
            c = {
              money: e,
              num: d,
  ...
  xhl: function() {
    ...
    var e = "a" + a + "b" + t + "c" + this.data.gg[this.data.xzggindex].id,
      s = this.data.xzggindex;
    if (2 == this.data.types) var o = this.data.gg[this.data.xzggindex].cost;
    else o = this.data.gg[this.data.xzggindex].cost;
    var d = this.data.dishes[t].goods[a].box_fee;
    console.log("餐盒费是：" + d);
    var i = Number(this.data.dishes[t].goods[a].num),
      n = this.data.dishes[t].goods[a].gg[this.data.xzggindex].num,
      r = this.data.ggbt + this.data.gg[this.data.xzggindex].name,
      l = this.data.dishes[t].goods[a].id,
      c = this.data.dishes[t].goods[a].img,
      g = this.data.store,
      h = {
        ggindex: s,
        money: o,
  ...
  addCart: function(a) {
    console.log(this.data);
    var h = this,
      u = a.currentTarget.dataset.itemIndex,
      p = a.currentTarget.dataset.parentindex;
    this.setData({
      zindex: u,
      findex: p
  ...
  payDesc: function() {
    console.log(this.data);
    var a = parseFloat(this.data.totalPrice),
      t = parseFloat(this.data.start_at); {
  ...
  clickImage: function(a) {
    ...
    console.log(e);
    for (var s = [], o = 0; o < t.data.dishes.length; o++)
      for (var d = 0; d < t.data.dishes[o].goods.length; d++)
        if (console.log(t.data.dishes[o].goods[d].id),
  ...
  bomb: function(a) {
    for (var t = a.currentTarget.id, e = this.data.dishes, s = 0, o = e.length; s < o; ++s)
      for (var d = e[s].goods, i = 0; i < d.length; i++) d[i].id == t ? d[i].open = !d[i].open : d[i].open = !1;
    this.setData({
      dishes: e,
